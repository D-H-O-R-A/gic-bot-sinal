...
exports.handleTokenInput = handleTokenInput;
...
async function handleTokenInput(ctx) {
    ...
    if (!/^0x[a-fA-F0-9]{40}$/.test(tokenAddress)) {
        return ctx.replyWithMarkdownV2('❌ *Invalid token address format*\n' +
    ...
    try {
        ...
        const result = await groupRepo.update({ chatId: String(ctx.chat.id) }, { tokenAddress: tokenAddress.toLowerCase() } // Normalizar endereço
        );
        if (result.affected === 0) {
            return ctx.reply('❌ Group not configured. Use /setup_group first');
        }
        await ctx.replyWithMarkdownV2(`✅ *Tracking token updated to:*\n\`${tokenAddress}\``, { parse_mode: 'MarkdownV2' });
    }
    catch (error) {
        // Tratamento type-safe de erros
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        await ctx.replyWithMarkdownV2(`❌ *Error updating token:* \`${errorMessage}\``);
    }
}
